...
function(h) {
    ...
    s = (_createClass(a, [{
        key: "init",
        value: function(a) {
            a && h.extend(!0, this.defaluts, a),
            this.buildCustomNavigation(),
            this.buildGithubCorner(),
            this.buildCopyright(),
            this.buildBloggerProfile(),
            this.modifyAdminButton(),
            this.isPostPage ? (this.buildPostCatalog(), this.buildPostCodeCopyBtns(), this.buildPostLightbox(), this.buildPostSignature(), this.buildPostSponsor(), this.buildToolbar(), this.buildPostCommentAvatars()) : this.showSideBar()
        }
    },
    {
        key: "showMessage",
    ...
    },
    {
        key: "showSideBar",
        value: function() {
            767 < h(window).width() && (h(this.cnblogs.forFlow).css({
                marginLeft: "22em"
            }), h(this.cnblogs.sideBar).fadeIn(600))
        }
    },
    {
        key: "modifyAdminButton",
        value: function() {
            var nav_admin = h(this.cnblogs.nav_admin_btn).text("Admin")
            var read_more = h(this.cnblogs.read_more_btn).text("Read More")
        }
    },
    {
        ...
        value: function() {
            var a = this,
            s = h(this.cnblogs.navList),
            n = h("body").css("color"),
            t = this.defaluts.navigation;
            if (t && t.length) {
                h.each(t.reverse(),
                function(a, t) {
                    if (t.chilren && t.chilren.length) {
                        var e = t.chilren.map(function(a) {
                            return '<li><a class="menu" href="' + a.url + '" target="' + t.target + '">' + a.title + "</a></li>"
                        });
            ...
            var o = h(".esa-search-box"),
            i = h(".esa-search-btn"),
            l = h(".esa-search-close");
            i.on("click",
            function() {
                o.slideDown("fast"),
                l.show(),
    ...
    },
    {
        ...
        value: function() {
            ...
            if (a.enable) {
                ...
                e = '<div class="esa-post-signature"> \n<p>Author: ' + (a.author || h(this.cnblogs.publicProfile).find("a:eq(0)").html()) + '</p> \n<p>From: <a href="' + t + '">' + t + '</a></p> \n<p>License: This work is licensed under a 「<a href="' + a.link + '"  target="_blank">' + a.license + "</a>」</p> \n<p>" + (a.remark || "") + "</p> \n</div>";
        ...
        value: function() {
            function a() {
                ...
                for (var a = h(o.cnblogs.feedbackContent), t = 0; t < a.length; t++) {
                    var e = "https://pic.cnblogs.com/face/sample_face.gif",
                    s = h(a[t]).find("span:last")[0];
                    s && (e = h(s).html().replace("http://", "//")),
                    h(a[t]).find(".esa-comment-avatar img").attr("src", e);
                    var n = h(a[t]).parent().find(".comment_date").next().attr("href");
                    h(a[t]).find(".esa-comment-avatar a").attr("href", n)
                }
            }
            var o = this;
            if (h(this.cnblogs.postCommentBody).length) a();
            else var t = 1,
            e = setInterval(function() {
                h(o.cnblogs.postCommentBody).length && (clearInterval(e), a()),
                10 == t && clearInterval(e),
                t++
            },
            500)
        }
    },
    {
        ...
        value: function() {
            var t = this.defaluts.sponsor,
            a = this.defaluts.github,
            e = this;
            if (t.enable) {
                ...
                s.find(".box>li").click(function() {
                    ...
                    if ("paypal" === a) {
                        if (!t.paypal) return e.showMessage("Haven't set PayPal QRCode");
                        i(t.paypal)
                    } else if ("alipay" === a) {
                        if (!t.alipay) return e.showMessage("Haven't set AliPay QRCode");
                        i(t.alipay)
                    } else if ("wechat" === a) {
                        if (!t.wechat) return e.showMessage("Haven't set WeChat Pay QRCode");
                        i(t.wechat)
                    }
                }),
                o.click(function() {
                    o.removeClass("showQR").addClass("hideQR"),
            ...
            function i(a) {
                a && o.css("background-image", "url(" + a + ")"),
                s.find(".text, .box, .github").addClass("blur"),
                n.fadeIn(300,
                function() {
                    o.addClass("showQR")
                })
    ...
    },
    {
        ...
        value: function() {
            ...
            if (l.enable) {
                var a = [l.level1, l.level2, l.level3],
                t = h(this.cnblogs.postBody).find(a.join(","));
                ...
                h.each(t,
                function(a, t) {
                    var e = h(t)[0].tagName.toLowerCase(),
                    s = h(t).text(),
                    n = "",
                    o = s;
                    if (l.index) e === l.level1 ? (d = c = 0, n = '<span class="level1">' + ++r + ". </span>") : e === l.level2 ? (d = 0, n = '<span class="level2">' + r + "." + ++c + ". </span>") : e === l.level3 && (n = '<span class="level3">' + r + "." + c + "." + ++d + ". </span>");
                    else switch (e) {
                    case l.level1:
                    ...
                    h(t).attr("id", "" + i).append('<a href="#' + i + '" class="esa-anchor">#</a>').hover(function() {
                ...
                p += "</ul>",
                e.find(".esa-catalog-contents").append(p),
                e.appendTo("body");
                var s = h(".esa-catalog-contents");
                if (s.fadeIn(), h(".esa-anchor-link").on("click",
                function() {
                ...
                function() {
                    ...
                    h(".esa-catalog-title").on("mousedown",
                    function(a) {
                        a.preventDefault(),
                        i.start = !0;
                        var t = h(".esa-catalog").position(),
                        e = a.clientX - parseFloat(t.left),
                        s = a.clientY - parseFloat(t.top);
                        i.pois = [e, s]
                    }),
                    h(document).on("mousemove",
                    function(a) {
                        if (i.start) {
                            var t = a.clientX - i.pois[0],
                            e = a.clientY - i.pois[1],
                            s = "fixed" === h(".esa-catalog").css("position");
                            a.preventDefault(),
                            i.stX = s ? 0 : h(window).scrollLeft(),
                            i.stY = s ? 0 : h(window).scrollTop();
    ...
    },
    {
        ...
        value: function() {
            var t = this,
            a = h(".postBody .cnblogs-markdown").find("pre");
            if (!a.length) return ! 1;
            h.each(a,
            function(a, t) {
                h(t).find("code").attr("id", "copy_target_" + a),
                h(t).prepend('<div data-tips="Copy" class="esa-clipboard-button" data-clipboard-target="#copy_target_' + a + '">Copy</div>')
            }),
            h.getScript(e.js,
            function() {
                ...
                a.on("success",
                function(a) {
                    t.showMessage("Copied"),
                    a.clearSelection()
                }),
                a.on("error",
                function(a) {
                    t.showMessage("Copy failed")
                })
            })
        }
    },
    {
        ...
        value: function() {
            var e = this,
            a = this.defaluts.catalog;
            h("body").append('<div class="esa-toolbar">\n<button class="esa-toolbar-gotop" data-tips="Back To Top"></button>\n<button class="esa-toolbar-contents" data-tips="List catalog"></button>\n<button class="esa-toolbar-follow" data-tips="Follow Me"></button>\n</div>');
            var t = h(".esa-toolbar-gotop"),
            s = h(".esa-toolbar-contents"),
            n = h(".esa-toolbar-follow");
            a.enable ? s.on("click",
            function() {
            ...
            function() {
                ...
                function() {
                    ...
                    function() {
                        ...
                        if (c_has_follwed) return e.showMessage("Have followed this author already.");
                        ...
                        h.ajax({
                            ...
                            success: function(a) {
                                "Please Login" == a ? login() : "Follow Successuflly!" == a && followByGroup(t, !0),
                                e.showMessage(a)
                            }
                        })
                    })
    ...
    },
    {
        ...
        value: function() {
            var a = this.defaluts.avatar,
            t = this.defaluts.favicon; ! this.isPostPage && a && h(this.cnblogs.sideBarMain).prepend('<img class="esa-profile-avatar" src="' + a + '" />'),
            t && h("head").append('<link rel="shortcut icon" href="' + t + '" type="image/x-icon" />')
        }
    },
    {
        ...
        value: function() {
            var a = this;
            h("head").append('<link rel="stylesheet" href="' + t.css + '"/>'),
            h.getScript(t.js,
            function() {
    ...
    },
    {
        ...
        get: function() {
            return {
                header: "#header",
                blogTitle: "#blogTitle",
                nav_admin_btn: "#blog_nav_admin",
                read_more_btn: "#c_b_p_desc_readmore",
